...
%    \end{itemize}
\item Wald Test
\item Lagrange Multiplier Test(Score Test)
\item \textbf{Score-based Tests(more general) w.r.t. ordinal variable}. 
\end{itemize}
\end{frame}
%  --------------------------------------------------------------------------
\begin{frame}
...
\begin{frame}
  ...
  \begin{itemize}
    \item The score-based tests \cite{MerZei13}
     ...
     \begin{column}{0.48\textwidth}
      ...
      $$\bordermatrix{\text{}& \theta_{1} & \theta_{2} & \ldots
        &\textcolor{red}{\theta_{k}}\cr
                ...
                x_{..}& \vdots & \vdots & \vdots & \vdots\cr
                x_{m-1}&  7.89 & 3.32 & \ldots & 4.91\cr
                x_{m}& 0     &   0     & \ldots & 0} $$}
        \end{block}
      \end{column}
   \end{columns}
...
\begin{itemize}
\item Statistics for ordinal variable: \\
...
\begin{frame}
  ...
  What is the tests' power under model misspecification? 
  \begin{itemize}
    \item The model missed out one parameter, which is \emph {not} the 
      M.I. parameter.
    \item The model missed out one parameter, which \emph{is} the M.I. 
      parameter. 
  \end{itemize}
\end{frame}
%--------------------------------------------------------------------------
...
\begin{frame}
\begin{figure}
    \caption{Missed out parameter is the M.I. parameter}
...
\end{enumerate}
\begin{itemize}
  \item Gratitude scale (GQ-6, 5 items) completed by six age
...
<<sctest-ord2, eval=TRUE>>=
 library(strucchange)
wdmo <- sctest(restr, order.by = yg$agegroup, parm = 1:4, 
               vcov = "info", functional = "WDMo",  plot = FALSE, 
...
\end{frame}
%--------------------------------------------------------------------------
\begin{frame}[shrink=0.8]{References}
