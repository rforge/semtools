  ...
  \begin{itemize}
    ...
    \item Notable examination of measurement invariance:
      \begin{itemize}
         ...
         \item Intelligence tests for different cohorts. \cite{wich04}
      \end{itemize}
  \end{itemize}
\end{frame}
%--------------------------------------------------------------------------
 ...
 Hypothesis of  measurement invariance and alternative:
  ...
  \begin{column}{0.48\textwidth}
    \begin{block}{G Groups}
      ...
      % \end{center}   
       $V_{g}$ can be categorical, \textit{ordinal} and continous.
    \end{block}
  \end{column}
\end{columns}
...
\item Lagrange Multiplier Test (Score Test) (Require estimates under
  null hypotheis)
\item \textbf{Score-based Tests (more general) w.r.t. auxiliary
...
\begin{frame}
  \frametitle{Empirical Cumulative Sum Score:  Systematic Order and 
    Decorrelate}
 We need a way to aggregate scores across people so that we
      ...
      \begin{itemize}
        ...
        \item Decorrelate sores for each parameter.    
      \end{itemize}
\begin{equation} 
    \label{eq:cumscore}
...
\begin{frame}
  ...
  \begin{columns}
    \begin{column}{0.48\textwidth}
      \begin{block}{Score}
        \resizebox{5.0cm}{!}{
        $$\bordermatrix{\text{}& \theta_{1} & \theta_{2} & \ldots 
          &\textcolor{red}{\theta_{k}}\cr
                x_{1}&  1.32  & -4.32 & \ldots & -2.34\cr
                x_{2}&  3.37 & 8.32 & \ldots & 63.45\cr
                x_{3}&  4.12 & 3.48 & \ldots & -3.11\cr
                x_{..}& \vdots & \vdots & \vdots & \vdots\cr
                x_{n-1}&  12.34 & 13.32 & \ldots & 6.89\cr
                x_{n}& -5.78     &   3.50     & \ldots & 7.67} $$}
        \end{block}
     \end{column}
     \begin{column}{0.48\textwidth}
       \begin{block}{Empirical Cumulative Score}
         \resizebox{5.0cm}{!}{
      $$\bordermatrix{\text{}& \theta_{1} & \theta_{2} & \ldots
        &\textcolor{red}{\theta_{k}}\cr
                x_{1}&  1.32  & -4.32 & \ldots & -2.34\cr
                x_{2}&  4.69 & 4.00 & \ldots & 61.11\cr
                x_{3}&  8.81 & 7.48 & \ldots & 58.00\cr
                x_{..}& \vdots & \vdots & \vdots & \vdots\cr
                x_{n-1}&  7.89 & 3.32 & \ldots & 4.91\cr
                x_{n}& 0     &   0     & \ldots & 0} $$}
        \end{block}
      \end{column}
   \end{columns}
...
\begin{itemize}
  \item Statistics for auxiliary categorical variable:\\
maximum Lagrange multiplier categorical ($\mathit{LM}_{uo}$), 
asymptotic equvialent to LRT.\\
\item Statistics for auxiliary ordinal variable: \\
Weighted Double Maximum ordinal ($\mathit{WDM}_o$),\\
maximum Lagrange multiplier ordinal ($\max\mathit{LM}_o$). \\
\item Statistics for auxiliary continuous variable:\\
Double maximumum statistic ($\mathit{DM}$)\\
...
\begin{frame}
  ...
  \begin{block}{Practice Consideration}
    \begin{itemize}
      \item Model misspecification is inevitable in practice.  
      \item M.I. is often studied with respect to \textbf{ordinal} variable.
...
\begin{frame}
  ...
  \begin{block}{Simulation:}
    What is the ordinal tests' power under model misspecification? 
    \begin{itemize}
      \item The estimated model excludes one parameter, which is \emph
        {not} the problematic parameter.
...
\begin{frame}
  \begin{figure}
    \caption{Excluded out parameter is not the problematic parameter}
...
\begin{frame}
\begin{figure}
    \caption{Excluded parameter is the problematic parameter}
...
\tiny{\caption{The parameter violating measurement invariance is 
    the unmodeled loading. Testing parameters are $\phi_{12}$ 
    and $\mu_{11}$.}}
\label{fig:sim24res}
...
\begin{frame}
 ...
 \begin{itemize}
   \item
      The ordinal test statistic proposed have better power 
   ...
   \item This feature is robust as long as the violation 
...
\begin{enumerate}[2.]
  \suite
\item Fit your constrained model: no need to comparing various models.
\end{enumerate}
<<modeest,eval=FALSE>>=
 library(lavaan)
  restr <- cfa("f1 =~ gq6_1 + gq6_2 + gq6_3 + gq6_4 + gq6_5",
...
sctest(restr, order.by = yg$agegroup, parm = 1:4, vcov = "info",
  functional = "WDMo", plot = TRUE, 
               ylim = c(0, 3.2), xlab = "Age group", 
               ylab = "Weighted test statistics")
@
\end{figure}
\end{frame}
%--------------------------------------------------------------------------
...
%--------------------------------------------------------------------------
\appendix
\begin{frame}[shrink=0.3]{Reference}
