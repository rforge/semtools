...
\text{L}({\bm{x}}_i, {\bm{\theta}}) \big |_{\bm{\theta} =
  ...
  \hat{\bm \theta}=\argmax_{\bm \theta} \ell(\bm \theta; x_{1}, \ldots, x_{n})\\
...
\begin{frame}
  \frametitle{Cumulative Sum of Scores}
      \begin{itemize}
...
\begin{itemize}
  \item Categorical $V$: Maximum Lagrange multiplier test (asymptotically equvialent to LRT).
\item Ordinal $V$: Weighted double maximum test ($\mathit{WDM}_o$);\\
Maximum Lagrange multiplier test ($\max\mathit{LM}_o$).
\item Continuous $V$: Double maximumum test ($\mathit{DM}$)\\
...
\begin{frame}
 ...
 \begin{itemize}
   ...
   \item The use of these tests is convenient, requiring estimation of
...
\begin{frame}
\begin{figure}
\tiny{\caption{Fluctuation processes for the $\mathit{LM}_o$ statistic}}
\label{fig:ordres}
...
\vspace{0.3in}
 ...
 invariance: Use in practice. \emph{Quantitative Psychology and Measurement},
 5, 438. 
\end{frame}
%--------------------------------------------------------------------------
\appendix
