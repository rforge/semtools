...
measurement invariance w.r.t.\ continuous, categorical, and ordinal variables
\cite{mer93,DolMaa98,lubmut05}\readme{The new references do not work
  here (because they are unrelated to tests based on stochastic
  processes).  I will work on these references.}.
...
scenarios likely to be encountered in practice.  To our knowledge, this 
is the first detailed examination of the tests' properties. \seeme{add
this sentence to point the unique contribution.}
...
than others.  Each of the $k$ scores represents one model parameter,
...
This allows us to test a hypothesis of partial\seeme{I
think partial should be weak} measurement invariance
...
, the factor covariance ($\phi_{12}$).  Note that the
 ...
 violation\seeme{need Meridith reference?}, but it is still a parameter instability that can occur in
 this type of model.
