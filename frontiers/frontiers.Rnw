...
covariance \seeme{delete the ''not shown''} did not demonstrate this error, however.  In
terms of specific 
...
dose violate measurement invariance, however, the test could detect there existing measurement invariance in the model but assign this violation to other 
parameters that do not violate measurement invariance.  \seeme{add the point that the tests could detect measurement invariance}.  The impacted parameters include the error
...
which the hypothesized model includes all parameters of importance.  The tests are model-based and could not inform the researcher model misspecification. \seeme{The last sentence.}
