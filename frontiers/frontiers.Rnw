...
are sensitive to monotonic w.r.t. variable of research interest and 
less sensitive to non-monotonic violations\seeme{original sentence:
the development of test statistics that are sensitive to invariance 
violations of interest and insensitive to "anomalous" invariance violations}.  
The test statistics are specifically applicable to situations where one 
...
\cite{mer93,DolMaa98,lubmut05}\seeme{add these three refs.}.
...
$\sum_{i = 1}^n s(\hat \theta;x_{i}) = 0$. \seeme{delete this
...
for measurement 
invariance w.r.t.\ age.  If there exists measurement invariance w.r.t. age
\seeme{original sentence:
If the manifest variables violate measurement
...
operator), thus t could be interpreted as $1/n$, $2/n$, $3/n$, ...$n/n$.
\seeme{add from thus to n/n}. 
$x_{(i)}$ reflects the individual with the $i$-th smallest value of the 
auxiliary variable $V$.  So we replace the score component shown in 
Equation~\eqref{eq:score} by cumulative score component shown in 
Equation~\eqref{eq:cumscore} without changing dimension
\seeme{add from So to dimension}. 
...
Brownian bridge \cite{hjokon02}\seeme{add this ref}.  
...
The response scale is 1 to 9 \seeme{add the scale}. 
...
(Note, the researcher could fit any kind of reduced model, 
e.g. restrict the residuals to be equal.  For model 
specification code, please refer to \pkg{lavaan} package)
\seeme{Add this note}.
%
<<models>>=
...
small measurement invariance violations \cite{Ben80}.  That is, the above tests (LRT and  $\mathit{LM}_\mathit{uo}$ test) \seeme{add the LRT to test content}
