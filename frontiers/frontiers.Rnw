...
similar results, with unordered  $\max\mathit{LM}$ demonstrated lower powers across all situations. ordered $\max\mathit{LM}$ and double-max exhibited equivalent power curves because, when only one parameter is tested, the statistics are equivalent.
...
\readme{We need to compare the specific tests: the unordered test, which mimics the LRT, has lower power.}
...
misspecified. \readme{The plots have too many points; maybe we should only plot  every other diff value.  In the xyplot command, you could add the argument \verb+subset = diff %% .5 == 0+.}
