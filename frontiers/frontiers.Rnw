...
\readme{We need a better intro here.  The special topic is about measurement invariance, so most of this paragraph is obvious to everyone.  Maybe start with a remark about approximate invariance and try to tie our topic to that somehow.}
...
\readme{I just added LMuo to the sctest command and corrected the command below; the range statistic is different.}
...
differing from every other age group.  One solution to this problem would be the {\em approximate invariance} methods proposed by Muth\'{e}n and others.\readme{Ting: Can you get some approximate invariance references and put them here?}
...
\readme{Please make sure the text below is correct.  The previous text stated that the violation occurs in all loadings and all unique variances.}
...
\node[error] (E6) at (7,-2) {E6}
   ...
   edge [errorsling, bend right=100] node[right=0.1,text width=2cm]{\scriptsize{$\psi_{11}=22.5$}}(E6.45);
...
generated and tested. Four statistics were examined above. \readme{What four statistics?  Equation numbers would be helpful.} 
In all conditions, we
maintained equal sample sizes at each level of the ordinal variable. 

\subsection{Results}
\readme{When referring to statistics, it would be good to use the notation from Equations 6--8.  For example, instead of ``unordered $\max\mathit{LM}$'', you would have $\max\mathit{LM}_{uo}$.}
...
similar results, with unordered  $\max\mathit{LM}$ demonstrating lower power across all situations. ordered $\max\mathit{LM}$ and double-max exhibited equivalent power curves because, when only one parameter is tested, the statistics are equivalent. \readme{Should say that the unordered statistic is asymptotically equivalent to the LRT.}
...
misspecified. \readme{The plots have too many points; maybe we should only plot  every other diff value.  In the xyplot command, you could add the argument ``subset = diff \%\% .5 == 0''.}
...
\caption{Simulated power curves for the ordered 
      $\max\mathit{LM}$ test, the double-max test, and the unordered $\max\mathit{LM}$ test across three
      levels of the ordinal variable $m$ and measurement invariance 
      violations of 0--4 standard errors (scaled by $\sqrt{n}$). The parameter violating measurement invariance is $\lambda_{11}$.}
\label{fig:sim11res}
\setkeys{Gin}{width=\textwidth}
...
print(xyplot(power ~ diff | nlevels+pars, group = ~ test, data = sim1,
             subset = (parms=="loading" & 
                      pars %in% c("Loading1","All Loadings","All Errors")),
...
\seeme{I think it might be nicer to keep these plots all have three rows?(Not sure)}
...
\subsection{Method}
\readme{Make sure there is space after each period and comma, the beginning of each sentence is capitalized, etc.}
...
tests of the first loading and error variance parameter exhibited high power, reflecting a high Type I error rate.  Tests associated with the factor covariance did not demonstrate this error, however.

\readme{Need some comparison between the unordered and ordered tests.}
...
A similar pattern of specificity and power were observed when the unique variance and factor covariance paraameters violated measurement invariance; the figures associated with these results are shown in the Appendix.

\readme{We should say something about how the unordered test looks better when the unmodeled parameter violates MI.}
...
\readme{Need some text describing what this appendix contains.}
