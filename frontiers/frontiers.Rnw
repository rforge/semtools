...
#WDMo 
maxLMo<-sctest(reduced, as.numeric(yg$agegroup),parm=1:4, stat="maxLMo", fnl=opval1, fplot=TRUE)
#maxLMo 
@
\end{figure}
...
sim1.tmp <- sim1[sim1$test %in% c("ordmax","ordwmax","catdiff")&sim1$parms %in% c("loading"),]
...
sim1.tmp <- sim1[sim1$test %in% c("ordmax","ordwmax","catdiff")&sim1$parms %in% c("var"),]
sim1.tmp$test <- factor(sim1.tmp$test)
trellis.par.set(theme = canonical.theme(color = FALSE))
mykey <- simpleKey(levels(sim1.tmp$test), points = TRUE, lines = TRUE)
print(xyplot(power ~ diff | nlevels+pars, group = ~ test, data = sim1.tmp, 
...
sim1.tmp <- sim1[sim1$test %in% c("ordmax","ordwmax","catdiff")&sim1$parms %in% c("error"),]
sim1.tmp$test <- factor(sim1.tmp$test)
trellis.par.set(theme = canonical.theme(color = FALSE))
mykey <- simpleKey(levels(sim1.tmp$test), points = TRUE, lines = TRUE)
print(xyplot(power ~ diff | nlevels+pars, group = ~ test, data = sim1.tmp, 
...
sim2.tmp <- sim2[sim2$test %in% c("ordmax","ordwmax","catdiff")&sim2$parms %in% c("extra"),]
