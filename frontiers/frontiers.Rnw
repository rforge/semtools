...
recently-proposed measurement invariance tests that are based on the {\em scores} of a fitted model.  This family can be used to test for measurement invariance
...
developing psychometric scales. Violation of it could happen in any degree. 
Generally, researchers want to ignore minor violation parameters, e.g. random 
normally distributed deviations from a parameter value across groups. BSEM
(Bayesian Structural Equation Modelling) is proposed not to pick up this kind of 
violating parameters but only "real" violating ones by assigning priors instead of strictly 
constraining equality across groups. After identifying the violation parameters, 
those non-invariant parameters are freely estimated to ensure final correct estimation.

Instead of using this two-step Bayesian analysis procedure, we are applying a family of tests derived
from stochastic process. These tests have been recently proposed to identify specific parameters impacted by measurement invariance. However, they have only been used to a limited type of parameter violation. It is this paper's intent to provide a detailed examination 
of the tests' performance under practical research scenarios. Specifically, we will examine these tests
specificity in identifying the violation parameter without giving false positive results. Moreover, in practice model misspecification is an inevitable issue to a certain extent, we will demonstrate these tests
results under model misspecification condition. 
\seeme{What about this intro? too much BSEM?}
...
@
The above command specifies that we wish to test model parameter numbers 1 through 4, which are the factor loadings supplied by lavaan.  The ordering of parameters can be obtained via the command \verb+coef(reduced)+. 

Because our sample size is large, this likelihood ratio test is known to be sensitive to 
...
differing from every other age group.  One solution to this problem would be the {\em approximate invariance} methods proposed by Muth\'{e}n \cite{muthen13}\seeme{How to handle muthen's name in refs.bib file?}.
...
testing measurement invariance w.r.t.\ truly continuous variables.
...
violation occurred in one of three places: the factor loading associated with Scale 1 
 ...
 ($\epsilon_{11}$), or the factor covariance $\sigma_{12}$.  
...
generated and tested. Statistics from Equations~\eqref{eq:wdm}, ~\eqref{eq:maxlmo} and~\eqref{eq:lmuo} were examined. 
In all conditions, we
...
similar results, with $\max\mathit{LM}_{uo}$ demonstrating lower power across all situations. This statistic 
is asymptotically equivalent to the LRT as what we describe in introduction. $\mathit{WDM}_o$  and $\max \mathit{LM}_o$ exhibited equivalent power curves because, when only one parameter is tested, the statistics are equivalent. 
...
tests of the first loading and error variance parameter exhibited high power, reflecting a high Type I error rate.  Tests associated with the factor covariance did not demonstrate this error, however. In terms of specific statistic performance, $\max \mathit{LM}_o$ test and $\mathit{WDM}_o$ test demonstrated higher power than $\mathit{LM}_{uo}$ in each panel, especially with increasing levels. It is indicating ordered statistics is more inclined to pick up the unmodelled parameter violation in the manifest variables' associated loading and error terms. 
\seeme{Is this sentence enough to compare ordered and unordered statistics?}
...
A similar pattern of specificity and power were observed when the unique variance and factor covariance parameters violated measurement invariance; the figures associated with these results are shown in the Appendix. Also, the power of ordered statistics $\max \mathit{LM}_o$ test and $\mathit{WDM}_o$ test showed higher power than unordered $\mathit{LM}_{uo}$ in each panel. 
...
This Appendix demonstrated the results for Simulation 2 when there exists model misspecification (lacking one path from Scale 1 to Math). Figure ~\ref{fig:sim22res} to Figure ~\ref{fig:sim24res} showed the violation parameter is first loading $\lambda_{11}$, covariance $\sigma_{12}$ and 
first error term $\psi_{11}$, respectively. 
