...
equality constraints with informative prior distributions \cite<e.g.,>{muthen13}.  In this paper, we
...
\readme{Achim: Should $\ell$ be summed from 2 to $m$?  The second term
  in parentheses is undefined for $\ell=1$.}
\begin{equation}
    \label{eq:lmuo}
...
Bayesian, \emph{approximate invariance} methods described in the
...
violation magnitude in the 
factor loading $\lambda_{11}$, with panels reflecting the parameters 
being tested and $m$ and lines reflecting different statistics.
...
invariance, respectively. In these figures, we only show tests
associated with parameters that exhibited nonzero power curves.  For
example, in Figure~\ref{fig:sim11res}, we do not show tests associated
with unique variance parameters or the factor covariance.  This is
because these tests exhibited near-zero power for all values of $d$.
...
being tested (top) and levels of the ordinal variable $m$ (bottom).}
\label{fig:sim11res}
...
violations of 0--4 standard errors (scaled by $\sqrt{n}$). The
parameter violating measurement invariance is $\phi_{12}$. Panel
labels denote the parameter(s)  
being tested (top) and levels of the ordinal variable $m$ (bottom).}
\label{fig:sim12res}
<<sim12res, fig=TRUE, height=4, width=9, echo=FALSE>>=
trellis.par.set(theme = canonical.theme(color = FALSE))
mykey <- simpleKey(levels(sim1$test), points = TRUE, lines = TRUE)
...
measurement invariance.  These results are displayed in Figure~\ref{fig:sim21res}.  One can generally observe
that tests of the first loading and unique variance exhibited high
``power,'' which is actually a high Type I error rate here.  This Type I error also demonstrated when testing all loadings and all unique variances.  The result is shown in Appendix.\seeme{From ''This type I'' to ''Appendix'' are added to refer to the added Appendix Figure. }  Tests
associated with the factor 
covariance (not shown) did not demonstrate this error, however. In
terms of specific 
...
variance and other loadings associated with the manifest variable
associated with the unmodeled 
loading.  Thus, as for other tests of measurement invariance, it is important to study the extent to
...
being tested (top) and levels of the ordinal variable $m$ (bottom).}
\label{fig:sim21res}
<<sim21res, fig=TRUE, height=7, width=9, echo=FALSE>>=
trellis.par.set(theme = canonical.theme(color = FALSE))
...
\proglang{R} package \pkg{lavaan}.  Next, we examined these
...
In the reminder of the paper, we will first compare
these tests to traditional tests of measurement invariance.  We also
...
This Appendix contains results for Simulation 2, where there existed
...
results when the unmodeled loading\seeme{add this to refer to the added figure}, loading $\lambda_{11}$, covariance $\phi_{12}$, and  
...
being tested (top) and levels of the ordinal variable $m$ (bottom).}
\label{fig:sim24res}
<<sim24res, fig=TRUE, height=4, width=9, echo=FALSE>>=
trellis.par.set(theme = canonical.theme(color = FALSE))
mykey <- simpleKey(levels(sim2$test), points = TRUE, lines = TRUE)
