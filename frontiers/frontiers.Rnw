...
variables.  Other methods for studying measurement
invariance w.r.t.\ continuous variables include moderated factor
models \cite{bauhus09,moldol10,pur02} and factor mixture models
\cite{dolmaa98,lubmut05}.  These methods require a model of greater
complexity to be estimated, while the tests described here work solely
on the output of a traditional factor model.  See \citeA{MerZei13} for
further comparison of these methods.
...
for measurement 
invariance w.r.t.\ age.  If there exists measurement invariance
...
violation occurred in one of four places: the factor loading associated with Scale 1 
 ($\lambda_{11}$), the unique variance  ($\psi_{11}$)
 or intercept associated with Scale 1 ($\mu_{11}$)
, the factor covariance ($\phi_{12}$).  Note that the
...
Math). Figure~\ref{fig:sim22res} to Figure~\ref{fig:sim26res} display
results when the unmodeled loading, the loading $\lambda_{11}$, the
covariance $\phi_{12}$, the error term $\psi_{11}$ and intercept $\mu_{11}$
