...
\readme{Achim: I added a definition of ${\bm B}(\hat {\bm
    \theta})_{i_{0}j}$.  Please double-check.} 
...
In the reminder of the paper, we first compare
...
for evaluating the parameter instability tests. \proglang{R}~and the
packages \pkg{lavaan} and \pkg{strucchange} are
