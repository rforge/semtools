...
posthoc test to examine whether specific parameters appear to vary
...
less than zero (though the sign of the scores will also depend on whether a
...
all model parameters, as well as test statistics associated with
...
%% TODO equations of statistics?

Importantly, these statistics were derived for situations where
...
variable to enter into the cumulative sum at the same time.  Test
statistics analogous to those describe above are then computed, with
...
To be more specific, in the following section, we will illustrate with an example to demonstrate how to implement multi group analysis in lavaan package. For more details, see \cite{vande12}

\subsection{Multiple Group Analysis}
Froh et al\citeA{FroFan11} had a large sample of youth(n=1401, ranging from late childhood(10 years old) 
to late adolescent(19 years old) complete the three most widely used adult gratitude inventories, 
including Gratitude Questionnaire 6(GQ-6; \citeNP{MccEmm02}), Gratitude Adjective Checklist(GAC; \citeNP{MccEmm02})  and Gratitude, Resentment, Appreciation Test-Short Form (GRAT-Short Form); \citeNP{ThoWat03}). In this specific analysis, the authors were interested in whether the youth factor 
structure for GQ-6 is a tau equivalent model (factor loadings restricted to be equal across
levels of age)or a congeneric model(all parameters free for each level of age).
...
please see \cite{vande12}. Since in most cases researchers are 
...
anova(theta, thetaAB)
@
Likelihood test is sensitive to large observations (CITE), it is not surprising to 
have a significant result, although Froh et al\citeA{FroFan11} showed that tau equivalent
model has as good alternative fit indices as congeneric model. 
...
the Background section.  That is, we can fit a model whose parameters 
...
%% TO BE MOVED TO BACKGROUND
%% In practice, there are three test statistics can be used: 
...
To demonstrate these tests in R, we will use the youth gratitude data
and assume that age group is continuous.
...
Alternatively, we could use sctest.lavaan function to get the statistics value,
corresponding $p$ value and the plot ordered by age level. 
...
<<conti3resmaxLM>>=
maxLM<-sctest(theta, as.numeric(yg$agegroup), parm=1:4, stat="maxLM", fplot=FALSE) 
maxLM
@
...
%% TODO Move these details to Background section
...
maxLMo<-sctest(theta, as.numeric(yg$agegroup),parm=1:4, stat="maxLMo", 
