...
parameter violating measurement invariance is $\phi_{12}$.}
\label{fig:sim12res}
...
``power,'' which is actually a high Type I error rate here.  Tests
...
Math). Figure~\ref{fig:sim22res} to Figure~\ref{fig:sim24res} display
results when the loading $\lambda_{11}$, covariance $\phi_{12}$, and  
...
being tested (top) and levels of the ordinal variable $m$ (bottom).}
\label{fig:sim23res}
<<sim23res, fig=TRUE, height=4, width=9, echo=FALSE>>=
trellis.par.set(theme = canonical.theme(color = FALSE))
mykey <- simpleKey(levels(sim2$test), points = TRUE, lines = TRUE)
