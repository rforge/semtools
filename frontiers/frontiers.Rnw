...
In this paper, we first described a novel family of test statistics for measurement invariance and illustrated their use via the R package lavaan. Next, we examined these statistics' ability to specifically pick up the violation parameter and specificity robustness to model specification. We found the proposed statistics could select the modeled parameter violating measurement invariance even though the model is misspecified in general. 

In the reminder of the paper, we will first consider the applicability of these tests in exploring the reason of measurement invariance. Next, the extension of these tests to other fit functions and more specialized models will be discussed. 
\seeme{Are these two paragraphs OK?}
