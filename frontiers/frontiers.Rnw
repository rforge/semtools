...
(though extension to other discrepancy functions is straightforward).
The tests are carried out after model estimation, making use of output
...
``posthoc'' test to examine whether certain parameters instead vary
...
tests CITE, and, in fact, these statistics can be viewed as  
...
$\hat{\theta}$ is the maximum likelihood estimate.  In words, each
...
%% Next: How scores can be used for testing
