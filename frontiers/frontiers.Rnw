...
tests that are based on the scores of a fitted model.  This family can be used
...
data, and (ii) whether the tests can accurately identify measurement invariance
violations in the presence of model misspecification.  After providing an
...
corresponding tests of measurement invariance.  BSEM (Bayesian structural
equation modeling) generally handles these ideas by replacing across-group
equality constraints with informative prior distributions.  In this paper, we
...
generally be used to study measurement invariance w.r.t.\ continuous,
categorical, and ordinal variables.

The family of tests described in this paper have recently been studied in the
context of measurement invariance \cite{MerZei13,MerFanZei}, though their
...
in practice and a detailed examination  of the tests' performance under
...
applied to SEM; for a more detailed account, see \citeA{MerZei13} and
...
s(\theta;x_{i}) = \left (
  ...
  \frac{\partial \ell(\theta;x_{i})}{\partial \theta_{k}} \right )^\top,
\end{equation} 
where $\ell(\theta; x_i)$ is the likelihood associated with individual
$i$, $\theta$ is a $k$-dimensional parameter vector. The corresponding
...
To formalize the ideas in the previous paragraph, we define a
...
\end{equation}
and it is asymptotically equivalent to the usual, likelihood ratio test statistic.
It is advantageous over the likelihood ratio test because it requires estimation of
...
package \emph{psychotools} \cite{zeilei12}.  The data consist of responses to three adult 
...
model \code{restr} after ordering the observations along the \code{agegroup} and
the observed information matrix is used as the variance-covariance matrix. Note
that the model parameters 1--4 are the factor loadings supplied by \pkg{lavaan}
as can be seen by inspecting \code{coef(restr)}. This also leads to somewhat
smaller test statistics that is very close to being significant at the 5\% level.
...
\emph{approximate invariance} methods proposed by Muth\'{e}n \cite{muthen13}.
...
Again, we employ the \code{sctest()} function to assess the parameters~1--4
...
both $p$-values exactly, the \proglang{R}'s random seed needs to be
...
In addition to test statistics, ``instability plots'' can be generated by additionaly setting
\code{plot = TRUE} in the \code{sctest()} calls above. The resulting plots representing
the statistics' fluctuations across levels of age group are displayed in
...
Factor analysis models(and SEMs) are usually the first step for social scientists to determine the latent construct. These models are fairly complex with many inter-related parameters. Previous applications of the proposed tests have typically involved simpler models. In this paper we intended to study whether the tests can still identify the specific parameters that violate measurement invariance when parameter correlation is present in the model. \seeme{the above part is added motivation. Please check.} Particularly, 
...
and~\eqref{eq:lmuo} were examined. In all conditions, we maintained equal sample
...
being tested and lines reflecting different statistics.
...
invariance, respectively. In those Figures, we only show test parameters demonstrating non-zero powers for clarity. \seeme{Add this sentence to explaining there is no zero power curves.}
All three test statistics exhibit
...
invariance violation. This ''dampening'' effect is more apparent for $\max \mathit{LM}_o$ statistic, because it sums across all parameters tested and then take the maximum. Whereas $\mathit{WDM}_o$ takes the maximum over parameters being tested directly.\seeme{Add from ''This'' to ''directly''.Please check.} 
...
\begin{figure}
\caption{Simulated power curves for $\max \mathit{LM}_o$ test, $\mathit{WDM}_o$ test, and
$\mathit{LM}_{uo}$ test across three levels of the ordinal variable $m$ and measurement invariance 
violations of 0--4 standard errors (scaled by $\sqrt{n}$). The parameter violating measurement 
invariance is $\lambda_{11}$.}
\label{fig:sim11res}
<<sim11res, fig=TRUE, height=9, width=9, echo=FALSE>>=
...
xyplot(power ~ diff | nlevels + pars, group = ~ test, data = sim1,
       ...
       type = "b", xlab = "Violation Magnitude", ylab = "Power", key = mykey, as.table = TRUE,
...
violations of 0--4 standard errors (scaled by $\sqrt{n}$). The parameter violating measurement
invariance is $\phi_{12}$.}
\label{fig:sim12res}
<<sim12res, fig=TRUE, height=5.5, width=9, echo=FALSE>>=
trellis.par.set(theme = canonical.theme(color = FALSE))
mykey <- simpleKey(levels(sim1$test), points = TRUE, lines = TRUE)
xyplot(power ~ diff | nlevels, group = ~ test, data = sim1,
       subset = (parms == "var" & 
...
\label{fig:sim13res}
<<sim13res, fig=TRUE, height=9, width=9, echo=FALSE>>=
trellis.par.set(theme = canonical.theme(color = FALSE))
mykey <- simpleKey(levels(sim1$test), points = TRUE, lines = TRUE)
xyplot(power ~ diff | nlevels + pars, group = ~ test, data = sim1,
       subset = (parms == "error" & 
...
\subsection{Results}
Results are presented by the parameter that violated measurement invariance. Of
primary interest is Figure~\ref{fig:sim21res}, which displays results when the
unmodeled loading violates  measurement invariance.  One can generally observe
that  tests of the first loading and error variance parameter exhibited high
power, reflecting a high Type I error rate.  Tests associated with the factor
covariance did not demonstrate this error, however. In terms of specific
statistic performance, $\max \mathit{LM}_o$ test and $\mathit{WDM}_o$ test
...
the misspecification. A similar pattern of specificity and power were observed
when the unique variance and factor covariance parameters violated measurement
invariance; the figures associated with these results are shown in the Appendix.
Also, the power of ordered statistics $\max \mathit{LM}_o$ test and
...
xyplot(power ~ diff | nlevels + pars, group = ~ test, data = sim2,
       subset = (parms == "extra" & 
		 pars %in% c("Loading1", "Error1") & diff %% 0.5 == 0),
       type = "b", xlab = "Violation Magnitude", ylab = "Power", key = mykey, as.table = TRUE,
       strip = function(..., which.given, factor.levels){
	   if(which.given == 2){
...
Many of the applications in this volume, along with many applications in
general, focus on testing for measurement invariance across unordered categories
...
detecting measurement invariance violations. \readme{In the abstracts for the
...
The tests are convenient for this purpose, as they do not require a new model to
...
potentially applicable to alternative SEM discrepancy functions such as weighted
...
calculation certainly could be implemented.  Test statistic calculation and
...
for evaluating the parameter instability tests. {R}~and the packages \pkg{lavaan} and 
\pkg{strucchange} are
...
This Appendix includes the results for Simulation 2 when there exists model misspecification (lacking one path from Scale 1 to Math). Figure~\ref{fig:sim22res} to Figure~\ref{fig:sim24res} display results when the loading $\lambda_{11}$, covariance $\phi_{12}$, and 
...
\label{fig:sim22res}
<<sim22res, fig=TRUE, height=9, width=9, echo=FALSE>>=
trellis.par.set(theme = canonical.theme(color = FALSE))
mykey <- simpleKey(levels(sim2$test), points = TRUE, lines = TRUE)
xyplot(power ~ diff | nlevels + pars, group = ~ test, data = sim2,
...
\begin{figure}
\caption{Simulated power curves for $\max \mathit{LM}_o$ test, $\mathit{WDM}_o$ test, and $\mathit{LM}_{uo}$ test across three levels of the ordinal variable $m$ and measurement invariance 
violations of 0--4 standard errors (scaled by $\sqrt{n}$). The violation parameter is $\phi_{12}$.}
\label{fig:sim23res}
<<sim23res, fig=TRUE, height=5.5, width=9, echo=FALSE>>=
trellis.par.set(theme = canonical.theme(color = FALSE))
mykey <- simpleKey(levels(sim2$test), points = TRUE, lines = TRUE)
xyplot(power ~ diff | nlevels, group = ~ test, data = sim2,
...
\label{fig:sim24res}
<<sim24res, fig=TRUE, height=9, width=9, echo=FALSE>>=
trellis.par.set(theme = canonical.theme(color = FALSE))
mykey <- simpleKey(levels(sim2$test), points = TRUE, lines = TRUE)
xyplot(power ~ diff | nlevels + pars, group = ~ test, data = sim2,
       subset = (parms == "extra" & 
