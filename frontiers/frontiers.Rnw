...
\seeme{Is it a different paper from vande12?}
...
Another similar statistics is unordered $\mathit{LM}_\mathit{uo}$ as shown in Equ. We can obtain this statistic by using the following function. 
<<Categorical>>=
## unordered LM
catdiff <- sctest(reduced, as.numeric(yg$agegroup), parm=1:4, stat="range")
round(c(catdiff$p.value), 2)
@
...
command \verb+coef(reduced)+.  We see that two of the three $p$-values output at 
...
if(file.exists("sim1.rda")) {
  ...
  levels(sim1$test) <- c("ordmax","ordwmax","catdiff")
...
sim1.tmp <- sim1[sim1$test %in% c("ordmax","ordwmax","catdiff")&sim1$parms %in% c("loading")&sim1$pars %in% c("Loading1","All Loadings","All Errors"),]
...
sim1.tmp <- sim1[sim1$test %in% c("ordmax","ordwmax","catdiff")&sim1$parms %in% c("var") &sim1$pars %in% c("All Loadings", "Covariance","All Errors") ,]
sim1.tmp$test <- factor(sim1.tmp$test)
trellis.par.set(theme = canonical.theme(color = FALSE))
mykey <- simpleKey(levels(sim1.tmp$test), points = TRUE, lines = TRUE)
print(xyplot(power ~ diff | nlevels+pars, group = ~ test, data = sim1.tmp, 
...
sim1.tmp <- sim1[sim1$test %in% c("ordmax","ordwmax","catdiff")&sim1$parms %in% c("error") &sim1$pars %in% c("Error1","All Errors","All Loadings"),]
sim1.tmp$test <- factor(sim1.tmp$test)
trellis.par.set(theme = canonical.theme(color = FALSE))
mykey <- simpleKey(levels(sim1.tmp$test), points = TRUE, lines = TRUE)
print(xyplot(power ~ diff | nlevels+pars, group = ~ test, data = sim1.tmp, 
...
if(file.exists("sim2.rda")) {
  ...
  levels(sim2$test) <- c("ordmax","ordwmax","catdiff")
...
sim2.tmp <- sim2[sim2$test %in% c("ordmax","ordwmax","catdiff")&sim2$parms %in% c("extra")&sim2$pars %in% c("Loading1","Error1","Covariance"),]
...
The core of the proposed ordinal test is to get the cumulative score process, which can be applied to any ML estimation functions or functions relating to maximizing or minimizing some statistics quantity.  
...
sim2.tmp <- sim2[sim2$test %in% c("ordmax","ordwmax","catdiff")&sim2$parms %in% c("extra+loading"),]
sim2.tmp$test <- factor(sim2.tmp$test)
trellis.par.set(theme = canonical.theme(color = FALSE))
mykey <- simpleKey(levels(sim2.tmp$test), points = TRUE, lines = TRUE)
print(xyplot(power ~ diff | nlevels+pars, group = ~ test, data = sim2.tmp, 
...
sim2.tmp <- sim2[sim2$test %in% c("ordmax","ordwmax","catdiff")&sim2$parms %in% c("extra+var"),]
sim2.tmp$test <- factor(sim2.tmp$test)
trellis.par.set(theme = canonical.theme(color = FALSE))
mykey <- simpleKey(levels(sim2.tmp$test), points = TRUE, lines = TRUE)
print(xyplot(power ~ diff | nlevels+pars, group = ~ test, data = sim2.tmp, 
...
sim2.tmp <- sim2[sim2$test %in% c("ordmax","ordwmax","catdiff")&sim2$parms %in% c("extra+error"),]
sim2.tmp$test <- factor(sim2.tmp$test)
trellis.par.set(theme = canonical.theme(color = FALSE))
mykey <- simpleKey(levels(sim2.tmp$test), points = TRUE, lines = TRUE)
print(xyplot(power ~ diff | nlevels+pars, group = ~ test, data = sim2.tmp, 
