...
\begin{frame}
  \frametitle{Score for Each Observation}
\begin{equation}
\label{eq:score}
s(\theta;x_{i}) = \left (
...
\begin{frame}
  \frametitle{Cumulative Sum Score: Decorrelate and Order}
\begin{equation} 
    \label{eq:cumscore}
...
of $n$, and $x_{(i)}$ reflects the 
individual with the $i$-th smallest value of the auxiliary variable $V$. 
  \end{frame}
%--------------------------------------------------------------------------
\begin{frame}
  ...
  \begin{itemize}
   \item  Under the hypothesis of measurement invariance, a central limit 
     theorem can be
   used to show that the fluctuation of the above cumulative sum score
...
%--------------------------------------------------------------------------  
 \begin{frame}
   \frametitle{Statistics Based on Cumulative Sum Socre}
   Multiple test statistics are available, depending on how one
summarizes the behavior of the cumulative sum of scores. Below shows
 two statistics for ordinal auxiliary variable with $m$ levels,  
...
\vspace{0.3in}
This statistic is asymptotically equivalent to the likelihood
ratio test statistic but only requires estimation of only one model 
(the reduced model).
 \end{frame}
%--------------------------------------------------------------------------
\begin{frame}
...
\section{Simulation}
\subsection{M.I. Violation Parameter is IN the Model}
\begin{frame}
 \begin{figure}
...
\begin{frame}
  ...
  Power and Type I error were examined across the following conditions.
 \begin{itemize}
...
\subsection{M.I. Violation Parameter is OUT the Model}
\begin{frame}
\begin{figure}
    \caption{Model used for the simulation.}
    \label{fig:pathinmodel}
...
\label{fig:ordres}
<<sctest-ord-graphics,fig=TRUE, height=3.3, width=5, echo=FALSE, results=hide>>=
