...
maxlmo <- sctest(restr, order.by = yg$agegroup, parm = 1:4,
  vcov = "info", functional = "maxLMo",  plot = TRUE, ylim = c(0, 14),
...
\begin{frame}
  \frametitle
\begin{figure}
\caption{Fluctuation processes for the $\mathit{WDM}_o$ statistic 
  (left panel) and the $\max\mathit{LM}_o$ statistic (right panel).}
\label{fig:ordres}
