...
\begin{frame}
  \frametitle{Empirical Cumulative Sum Score: Decorrelate and Order}
\begin{equation} 
    \label{eq:cumscore}
...
summarizes the behavior of the empirical cumulative sum of scores. Below 
shows two statistics for ordinal auxiliary variable with $m$ levels,  
