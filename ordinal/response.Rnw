...
lev0 <- c(   "LM_o","LRT","LRT_sb","AIC")
lev  <- c("maxLM_o","LRT","LRT_sb","AIC")
cl <- c(1, 4, 2, 3)
pc <- c(1, 2, 3, 4)
lt <- c(1, 1, 2, 2)
...
\begin{itemize}
  \item Both graphs are organized in the same manner as the graphs in the paper,
    ...
    ($\mathrm{LM}_o$), the Likelihood ratio test (LRT), the Satorra-Bentler LRT ($\mathrm{LRT}_\mathrm{sb}$),
    and the use of AIC (AIC). The former two are presented in the manuscripts while the latter
    two are only shown here.
...
substantial new insights and hence have omitted them from the paper (and only
