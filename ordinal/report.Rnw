...
psychometric models \citep{KopZei10,MerZei12}.
...
discussed in detail by \citet{MerZei12}. Here,
...
${\bm B}(t, \hat {\bm \theta})$.  In simulations, \citet{MerZei12} found 
...
matrix; see also \citet{hotzei08} for more details. In R, this can be
...
(see \citealp{MerZei12}, for related discussion), while the
...
@
\caption{\label{fig:sim1res}
      Simulated power curves for the ordered and unordered
...
of \citet{Ben80} and others that the
...
youth. Addressing this issue, \citet{FroFan11} had a
large sample of 
...
\citet{FroFan11} used confirmatory factor 
...
tests, \citet{FroFan11} examined a set of alternative fit indices, including
...
In the analyses described below, we re-analyze the \citet{FroFan11} data using
...
We focus on two analyses from \citet{FroFan11} where the likelihood
...
Of the 1401 cases originally collected by \citet{FroFan11}, we use here all
...
axis(2)
@
\caption{\label{fig:gq6res}
  Fluctuation processes for the $\mathit{WDM}_o$ statistic (left panel) and
...
model to the GQ-6 data, \citet{FroFan11} used alternative fit indices to
...
which supports the conclusions that \citet{FroFan11} obtained from
...
GAC data, \citet{FroFan11} obtained mixed results.  The alternative fit
...
for the data considered here).  \citet{FroFan11} ultimately 
...
As described in detail by \citet{MerZei12}, we
...
to the $\max\mathit{LM}$ statistic described in \citet{MerZei12}.
