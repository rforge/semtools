...
psychometric models \cite{KopZei10,MerZei13}.
...
discussed in detail by \citeA{MerZei13}.  Additionally, these ideas
are related to those underlying Modification Indexes (e.g.,
\citeNP{Sor89}): the Modification Index is equivalent to a Lagrange
multiplier test, and the Lagrange multiplier test is contained in the
family described by \citeA{MerZei13}.  Here,
...
\begin{equation} \label{eq:cumscore}
  {\bm B}(t; \hat {\bm \theta}) ~=~ \hat {\bm I}^{-1/2} n^{-1/2}
    \sum_{i = 1}^{\lfloor n t \rfloor} {\bm s}(\hat {\bm \theta}; x_{(i)})
  \qquad (0 \le t \le 1)
\end{equation}
...
the model (see \citeNP{hub67} for the theoretical foundation and
\citeNP{zei06b} for a computational framework).  
...
${\bm B}(t, \hat {\bm \theta})$.  In simulations, \citeA{MerZei13} found 
...
of ``invariant uniquenesses'' (see \citeNP{VanLan00}). % Or comparing
                                        % parallel to tau-equivalent 

The four ``traditional'' statistics that we consider generally treat
the ordinal auxiliary variable as categorical.  The first two are the
likelihood ratio test of measurement 
invariance in the six unique variance parameters and the
unordered LM test from~\eqref{eq:lmuo}.  Next, we consider the
Satorra-Bentler \citeyear{SatBen01} scaled likelihood ratio test
statistic with correction for difference testing.  Finally, we
consider the use of AIC \cite{Aka74} for detecting measurement
invariance.  In this context, we compute AIC for both the ``reduced'' 
...
(see \citeNP{MerZei13}, for related discussion), while the
...
statistics are all of the LM-type and just require estimation of the
single-group model, 
...
with 16 fewer parameters ($= 6 \cdot 4 - 2 \cdot 4$ because the four variances have to be estimated only in two rather than in six age groups).  The intermediate model also fits 
...
As described in detail by \citeA{MerZei13}, we
...
to the $\max\mathit{LM}$ statistic described in \citeA{MerZei13}.
...
for each dataset, which can be somewhat time-consuming.
...
presentation involving non-ordinal variables, see \citeNP{KopZei10}),
