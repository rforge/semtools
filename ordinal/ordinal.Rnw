...
\readme{Several details/extensions added to the following three paragraphs.}
...
Note that the effect outer maximum can be easily captured by a Bonferroni
...
matrix; see also \citeNP{hotzei08} for more details. In R, this can be accomplished
using the mvtnorm package \cite{mvtnorm}.
...
more sensitive to changes that occur only in one out of many parameters
