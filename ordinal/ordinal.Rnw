...
treated as categorical so that the traditional tests can be applied.  Alternatively, if there are many levels of $V$, $V$ may also be treated as continuous. 
...
We  consider situations in which a $p$-dimensional variable $X$
...
measurement invariance w.r.t.\ continuous or ordinal $V$, and 
...
As stated previously, we focus here on situations where $V$ is
...
discussed in detail by Merkle and Zeileis \citeyear{MerZei12}. Here,
...
For the $\mathit{WDM}_o$ statistic, the resulting asymptotic distribution is
...
simultaneously across the unique variances, which is similar to
comparing a tau-equivalent model to a congeneric model (e.g.,
\citeNP{VanLan00}).

We also compute two statistics that treat the ordinal
...
in youth. One potential problem with this research is that
...
\citeNP{FroFan11} used confirmatory factor 
...
<<frohdata>>=
yg <- read.spss("../prelim_code/grat_data_combined_2009-09-29.sav", to.data.frame=TRUE)
...
m1 <- cfa('f1 =~ gq6_1 + gq6_2 + gq6_3 + gq6_4 + gq6_5',
          ...
          meanstructure=TRUE)
gefp1 <- gefp(m1, fit = NULL, scores=estfun.lavaan, 
...
\fixme{Here and also subsequently, formulations like ``we compare model~A
and model~B'' are used. I'm not sure whether this sends the correct message.
If I want to compare exactly these two models, then the LRT is the correct
thing to do. But we are looking for other types of deviations, especially if
a split into two (rather than all) groups might be sufficient to capture
the effect of $V$. Hence, maybe we should explain in some more detail what
we are testing and also explore other models when rejecting the model? For
more comments in this vein see below.}
\readme{Achim: Added two sentences here that hopefully address the issue.}
...
differ within each subgroup.  Instead, we test for deviations such
that a split into two subgroups is sufficient to capture the effect of
$V$.
In employing the ordinal tests, we obtain~$\mathit{WDM}_o=\Sexpr{round(omax1$statistic, digits = 2)}, p=\Sexpr{round(omax1$p.value, digits = 3)}$
 and~$\max\mathit{LM}_o=\Sexpr{round(ol2bb1$statistic, digits = 2)}, p=\Sexpr{round(ol2bb1$p.value, digits = 3)}$.
Both $p$-values are clearly larger than that of the likelihood ratio test
and neither is significant at an $\alpha$ of 0.05,
...
as an indication that some factor loading is somewhat unstable at an
...
m2 <- cfa('f1 =~ gac_1 + gac_2 + gac_3
           ...
           meanstructure=TRUE)
gefp2 <- gefp(m2, fit = NULL, scores=estfun.lavaan, 
              order.by = as.numeric(yg$agegroup), 
...
parameters differ between individuals up to 13 years of age and older
than 13 years. \fixme{Maybe we could additionally fit a tau-equivalent
model for only two age groups: $\le 13$ vs.\ $> 13$. Then, we could
re-run tests and check whether this already fits sufficiently well?
This could help making the split into two subgroups clearer, rather
than always splitting into all possible age groups.}
\readme{Achim: See code starting on line 963 of the Rnw file. Even if we
  split around age 13, the test statistics are all significant.}

<<gac.2grp>>=
yg <- transform(yg, under14=as.numeric(agegroup)<=2)

m3 <- cfa('f1 =~ gac_1 + gac_2 + gac_3
           f1 ~ 0*1', data=yg, group="under14",
           group.equal=c("loadings","residuals","lv.variances"), 
           meanstructure=TRUE)
gefp3 <- gefp(m3, fit = NULL, scores=estfun.lavaan, 
              order.by = as.numeric(yg$under14), 
              vcov = info_full, sandwich = FALSE, parm=3:6)
if(file.exists("ol2bb3.rda")){
  load("ol2bb3.rda")
} else {
  ol2bb3.fun <- ordL2BB(gefp3)
  save(ol2bb3.fun, file="ol2bb3.rda")
}
omax3 <- sctest(gefp3, functional=ordwmax(gefp3))
ol2bb3 <- sctest(gefp3, functional=ol2bb3.fun)

## Full model
m3.f <- cfa('f1 =~ gac_1 + gac_2 + gac_3
           f1 ~ 0*1', data=yg, group="under14",
           group.equal="loadings", meanstructure=TRUE)
lrt3 <- anova(m3.f, m3)
@ 
...
has an auxiliary ordinal variable and wishes to study measurement
...
general (non-monotonic) measurement invariance w.r.t.\ $V$. \readme{I
replace ``sharper'' by ``more powerful'' which would be the correct
statistical jargon. Or is sharper the terminus technicus in psychometrics?}
\readme{Achim: No, ``sharper'' was simply my terminus technicus when I
...
(especially if $V$ has many levels).  
\fixme{We should also point to the supLM test of our first paper here.
The maxLM and supLM will generally be very close if the number of
potential thresholds is large. Maybe we could also re-iterate that
the forming of age groups a priori might not even be necessary and
more studies could collect data about age as a continuous rather than
just an ordinal variable.}
\readme{Achim: You say both ``maxLM'' and ``supLM'' above, but I assume
  you are generally referring to maxLM for ordinal $V$ vs maxLM for
  continuous $V$.  See sentences below.}
As described in detail by Merkle and Zeileis \citeyear{MerZei12}, we
...
to the $\max\mathit{LM}$ statistic described in Merkle and Zeileis.
...
to collect continuous age data (e.g., measured by days instead of by 
years).
...
\cite{BauHus09,MolDol10,Pur02} and factor mixture models
\cite{DolMaa98,LubMut05}.  Under the moderated factor model approach,
...
continuous are likely to be advantageous, especially if the levels of
...
ordinal $V$, however, while that the methods we propose here are
...
\fixme{Should we make some practical remarks about the computation of
$p$-values somewhere? The issue with the maxLM test is that it is hard
to tabulate critical values in advance and hence simulation has to be
done for each case and is potentially rather time-consuming. Not sure
whether we should do this now or wait until the revision or so\dots}
\readme{Achim: I am happy to wait for the revision, and for now have
  inserted a TODO comment into the Rnw file.}

% TODO Practical remarks about the computation of p-values

\subsection{Summary}
The test statistics proposed in this paper have relatively-high power for
...
rejects $H_0$ in practice (e.g., \citeNP{Ben80}).  Furthermore, the
