...
of Bentler and Bonett \citeyear{BenBon80} and others that the
...
students aged 10 to 19 years.  Each of the scales were individually
factor-analyzed using the items comprising the scale.
For each model, the authors 
first fit a congeneric model (all parameters free for each age group),
followed by a tau-equivalent model (factor loadings restricted to be
equal across groups) and a parallel model (all parameters restricted
to be equal across groups).  Because their sample size was large ($n
...
series of tests that are less sensitive to sample size than the
likelihood ratio test, while being
more sensitive to ``ordinal'' violations of measurement invariance.
...
alternative fit measures indicated the opposite (using the rules of
thumb employed by Froh et al).  These include
...
fit the restricted model and test for instability in the focal
model parameters.
...
\paragraph{GQ6.} Ordinal tests do not support LRT.
...
\paragraph{GAC.} Ordinal tests do support LRT.
...
\begin{enumerate}
  \item The only proposed statistics that treat ordinal auxiliary
    variables as truly ordinal?  LRT treats them as categorical,
    inclusion in the model typically treats tham as continuous.
  \item The first point is important because, if we test wrt an
