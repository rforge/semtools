...
Simulation results are presented in Figure XX.  Rows of the figure
...
if(file.exists("sim2.rda")) {
  ...
  sim2 <- simulation(diff = seq(0, 0.5, by = 0.1), nobs = c(1200, 4800, 9600),
                     anomaly=TRUE)
...
ol2bb1 <- sctest(gefp1, functional=ordL2BB(gefp1))
...
ol2bb2 <- sctest(gefp2, functional=ordL2BB(gefp2))
