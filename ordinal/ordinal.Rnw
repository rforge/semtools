...
\readme{Maybe we should explicitly mention that in parametric settings, f is
the probability density function of x? Also, do we need to point out
the relation between X and x, V and v, T and t?}
...
psychometric model by fitting the model and examining whether or not
...
hypothesis is often tested against an alternative
\begin{equation}
  \label{eq:h1*}
...
$V$.  Note that this alternative hypothesis can also be used when $V$
is ordinal, testing whether the model parameters shift at some point
on $V$.  
Alternatively, if $V$ is categorical with a relatively-small number of
groups, a frequent alternative of interest is
\begin{equation}
  \label{eq:h2*}
  H_2^*:~ {\bm \theta}_i = {\bm \theta}_{v_i}.
\end{equation}
...
higher power to detect the ``ordinal'' types of violations described above.

\readme{My feeling is that we still need to explain better what type
of patterns are covered by which alternative. Maybe we should do it
the other way round and start with $H_2^*$ as the most general type
of deviation from $H_0$. Then, we could explain that these also
covers changes that are not `monotonic' in V, i.e., where parameter
values change in one direction as V increases and then change into
another direction again. After that we could continue and argue that
such monotonic changes are of particular interest for categorical V
and that the simplest form of such a change is $H_1^*$. And as a final
note one could add that this one-shift alternative is also often
employed in practice where practitioners often use the median V
as the cutoff whereas we want to assess all possible cutoffs.}
...
discussed by \citeNP{MerZei12} in detail. Here, we review the tests'
most important properties that are relevant for extending the tests
to the ordinal case.

To formalize these ideas, we assume that the observations are ordered
...
\readme{I changed/extended the comments about the power of the tests
and their relative advantages somewhat.}
...
related to multiple parameters fluctuating simultaneously, and
\citeNP{MerZei12} found it to be easy to visualize and
interpret but to have relatively-low power for assessing measurement
...
${\bm B}(t, \hat {\bm \theta})$.  In simulations, \citeNP{MerZei12} found
that both tests perform well when assessing simultaneous changes
in multiple factor analysis parameters with some advantages for the
$\mathit{CvM}$ test in their simulation setup. These simulations included
situations in which subsets of model parameters were tested; such situations are handled
...
discretely measured leading to many ties.) \readme{Extended the previous sentence somewhat.
My feeling is that we now should first comment on the ordinal test statistics because these
directly build on the previous `continuous V' section. Then we can comment later on
on the categorical case.}

\readme{I have also extended the comments about the nominal/categorical case.}
Furthermore, if $V$ is only nominal/categorical,
...
\fixme{The $i_\ell$ still need to be introduced. This would happen automatically
if we move the nominal statistics behind the ordinal statistics.}
...
m-1$) associated with the observed $V$. For $\mathit{WDM}_o$ this
just requires computations of quantiles from a multivariate normal
distribution with standard normal marginals and a certain correlation
structure \cite{hotzei08} which can be easily obtained using R's
mvtnorm package \cite{mvtnorm}. For $\max \mathit{LM}_o$, no
closed-form solution for this particular functional of a Brownian bridge is unavailable, but
...
As demonstrated in the simulations below, the test
...
power greater than .05 at $d=0$).  This is because the likelihood
ratio test requires 
estimation of a multiple groups model, which is very unstable with
large numbers of groups and small sample sizes.  The proposed
statistics, which require estimation of only the single-group model,
exhibit a clear advantage in these conditions.
...
invariance.  The $\text{LM}_o$ statistic has higher power to detect
...
youth. Addressing this issue, Froh et al.\ \citeyear{FroFan11} had a
large sample of 
...
Froh et al.\ \citeyear{FroFan11} used confirmatory factor analysis
...
(i.e., chi-square difference tests) for model comparison because the
tests were overly sensitive at their sample size.  To supplement these
tests, Froh et al examined a set of alternative fit indices, including
...
In the analyses described below, we re-analyze the Froh et al.\ data using
...
We focus on two analyses from Froh et al.\ where the likelihood
...
alternative fit measures employed by Froh et al.\ indicated the opposite.
These include
...
Of the 1401 cases originally collected by Froh et al., we use here all
...
model to the GQ-6 data, Froh et al.\ used alternative fit indices to
...
Neither statistic is significant at an $\alpha$ of .05,
which supports the conclusions that Froh et al.\ obtained from
...
GAC data, Froh et al.\ obtained mixed results.  The alternative fit
...
model ($\chi^2_{20} = \Sexpr{round(lrt2[[5]][2],1)}, p<.01$
for the data considered here).  Froh et al.\ ultimately 
...
This results in~$\text{WDM}_o=\Sexpr{round(omax2$statistic,1)}, p<.01$
and~$\text{LM}_o=\Sexpr{round(ol2bb2$statistic,1)}, p<.01$.
...
\begin{enumerate}
  \item To my knowledge, these are the only proposed statistics that
    treat ordinal auxiliary 
    variables as truly ordinal.  LRT treats them as categorical,
    inclusion in the model typically treats tham as continuous.
  \item The first point is important because, if we test w.r.t.\ an
    ordinal auxiliary variable, we are generally looking for ordinal
    violations.  We don't care if a middle level differs from
    all others because we would not usually have an intuitive
    explanation for it.
  \item If we know our sample size is large and have an ordinal
    auxiliary variable, should we even use the LRT in
    the first place?  If the measurement invariance violation is truly
    ordinal, the proposed statistics should be better.  If it is not
    ordinal, the violation will be difficult to explain as anything
    but an anomaly.  The stakes might also play a role here.  In
    high-stakes testing, you might be more interested in treating the
    ordinal variable as categorical to detect any possible
    instability, even those that don't make sense.
  \item The proposed statistics should generally transfer to IRT and
    other psychometric models.
\end{enumerate}
