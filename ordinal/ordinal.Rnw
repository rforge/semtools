...
\abstract{
  ...
  of the auxiliary variable and hence have higher power against ``monotonic''
  violations of measurement invariance and less power against ``non-monotonic''
  ones. \readme{I thought a comment like this might help. But I'm not sure
  whether ``(non-)monotonic'' is intuitive to all readers.}  
...
\readme{$\max \mathit{LM}_o$ is now consistently employed as the name
for this statistic, also in the graphics. First, this is more consistent
with the notation in the theory section. Second, it should remind the
reader that this is not an ordinary LM test but an aggregation of
LM statistics.}
...
\readme{Ed: For now, I added the phrase ``across multiple model parameters'' above as a brief reference to this issue.}
\readme{Z: OK, I appended ``in factor analysis models'' to make the statement somewhat less general and more specific to our results.}
...
not the factor loadings fluctuate with respect to age group.
\fixme{Here, I would prefer something like: We can use the proposed
ordinal statistics to assess whether or not the factor loadings in the
tau-equivalent model are in fact stable or fluctuate with respect to age.}
In doing so, we obtain~$\mathit{WDM}_o=\Sexpr{round(omax1$statistic, digits = 2)}, p=\Sexpr{round(omax1$p.value, digits = 3)}$
...
up some non-systematic dependence on age. \readme{I have changed the
previous sentence somewhat to make clear that the LRT really picks up
something but that it's not what we are looking for.}
...
in a single group and hence are always zero.
...
\readme{Added some more detail about how to read the visualizations of
the test statistics. Modified the interpretations somewhat.}
...
large sample size. \readme{Sentence somewhat modified.}
...
only ones that treat auxiliary variables as ordinal and thus direct power against
alternatives that are typically of interest to practitioners
\readme{added half-sentence}.  Other methods
...
clearly less powerful than the likelihood ratio test \readme{added some
...
This treatment would allow the researcher to employ moderated factor models
\cite{BauHus09,MolDol10,Pur02} or factor mixture models
...
theorem). \readme{Added the comment in brackets. Useful or too detailed and
potentially confusing?} The only
