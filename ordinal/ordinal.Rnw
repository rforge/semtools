...
discussed in detail by \citeA{MerZei12}. Here,
...
${\bm B}(t, \hat {\bm \theta})$.  In simulations, \citeA{MerZei12} found 
...
matrix; see also \citeA{hotzei08} for more details. In R, this can be
...
(see \citeNP{MerZei12}, for related discussion), while the
...
of \citeA{Ben80} and others that the
...
youth. Addressing this issue, \citeA{FroFan11} had a
large sample of 
...
\citeA{FroFan11} used confirmatory factor 
...
tests, \citeA{FroFan11} examined a set of alternative fit indices, including
...
In the analyses described below, we re-analyze the \citeA{FroFan11} data using
...
We focus on two analyses from \citeA{FroFan11} where the likelihood
...
Of the 1401 cases originally collected by \citeA{FroFan11}, we use here all
...
model to the GQ-6 data, \citeA{FroFan11} used alternative fit indices to
...
which supports the conclusions that \citeA{FroFan11} obtained from
...
GAC data, \citeA{FroFan11} obtained mixed results.  The alternative fit
...
for the data considered here).  \citeA{FroFan11} ultimately 
...
than 13 years. \fixme{Z: Maybe we could additionally fit a tau-equivalent
...
\readme{ECM: See code starting on line 963 of the Rnw file. Even if we
  split around age 13, the test statistics are all significant.}
\readme{Z: My formulation above was misleading. We started out from
model m2 with 24 parameters where variances are restricted to be equal
across all groups. The LRT compares this to model m2.f with 44 parameters,
where the group-specific variances for all 6 age groups are estimated.
This ``costs'' us $(6 - 1) \cdot 4 = 20$ additional parameters. Instead,
we could fit a model m2.2 with 4 additional parameters -- i.e., 28 in total --
where the variances are just estimated separately for two age groups (split at age 14).
This should improve significantly upon m2 but if we are lucky it is not
significantly worse than m2.f.}
...
As described in detail by \citeA{MerZei12}, we
...
to the $\max\mathit{LM}$ statistic described in \citeA{MerZei12}.
...
% TODO % \fixme{Z: Should we make some practical remarks about the computation of
% TODO % $p$-values somewhere? The issue with the maxLM test is that it is hard
% TODO % to tabulate critical values in advance and hence simulation has to be
% TODO % done for each case and is potentially rather time-consuming. Not sure
% TODO % whether we should do this now or wait until the revision or so\dots}
% TODO % \readme{ECM: I am happy to wait for the revision, and for now have
% TODO %   inserted a TODO comment into the Rnw file.}
