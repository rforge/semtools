...
preferred to the $\text{WDM}_o$ statistic for simultaneously assessing
measurement invariance across multiple model parameters.  
\readme{This is not surprising given that the models
...
\readme{Achim: I decided not to set up a case where a single parameter
changes, because measurement invariance researchers usually test
multiple parameters simultaneously.  For now, I added the phrase "across
multiple model parameters" above as a brief reference to this issue.}
...
\begin{figure}
\caption{Fluctuation processes for the $\text{WDM}_o$ statistic (left panel) and
  the $\text{LM}_o$ statistic (right panel), arising from the GQ-6 data.}
\label{fig:gq6res}
\setkeys{Gin}{width=\textwidth}
<<gq6res, fig=TRUE, height=3, width=6>>=
par(mfrow=c(1,2), cex=.65)

plot(gefp1, functional=ordwmax(gefp1), axes=FALSE, main="",
     xlab="Age group", ylab=expression("WDM"[o]))
axis(1, at=1:5, labels=c("10-11","12-13","14","15","16"))
axis(2)

plot(gefp1, functional=ol2bb1.fun, axes=FALSE, ylim=c(0,14), main="",
     xlab="Age group", ylab=expression("LM"[o]))
axis(1, at=1:5, labels=c("10-11","12-13","14","15","16"))
axis(2)
@
\end{figure}
...
displays the process associated with $\text{WDM}_o$, while the right
panel displays the process associated with $\text{LM}_o$.  The final
age group (17-19 years) is not displayed, because the statistics
associated with this final age group are always zero (scores are
summed across all individuals in the sample at this point).
Critical values are displayed as horizontal red lines, so that the
hypothesis of invariance is rejected if the process ever crosses the
red line.
It is observed that both statistics generally increase with age, with
$\text{WDM}_o$ being largest in the 15-year group and $\text{LM}_o$
being largest in the 16-year group.  The $\text{WDM}_o$ panel displays
(a function of) the largest
cumulative sum associated with a single parameter, while the
$\text{LM}_o$ panel aggregates across the tested parameters.  The
differing pattern of values for the 15- and 16-year-olds can be taken
as evidence that a single factor loading is relatively unstable for the
15-year-olds, but this is not a general trend across all the tested
loadings.
...
\begin{figure}
\caption{Fluctuation processes for the $\text{WDM}_o$ statistic (left panel) and
  the $\text{LM}_o$ statistic (right panel), arising from the GAC data.}
\label{fig:gacres}
\setkeys{Gin}{width=\textwidth}
...
Plots representing the statistics' fluctuations across levels of age
group are displayed in Figure~\ref{fig:gacres}.  The left panel
displays the process associated with $\text{WDM}_o$, while the right
panel displays the process associated with $\text{LM}_o$.  It is
...
processes peak at the 12-13 age group.  This implies that fluctuation 
in the variance parameters is largest within this age group, with the 
parameters differing between individuals younger and
older than 12 years.
...
(especially if $V$ has many levels).  This treatment would allow the
researcher to employ moderated factor models
