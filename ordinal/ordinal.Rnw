\documentclass[man,mask,apacite]{apa6}
...
\authornote{This work was supported by National Science
...
measurement invariance w.r.t.\ categorical or continuous $V$, and 
...
As stated previously, we specifically focus here on situations where $V$ is
...
statistic with correction for difference testing, the Yuan-Bentler
\citeyear{YuaBen97} scaled test statistic, and the use of AIC
\cite{Aka74} for detecting measurement 
invariance.  We do not report the latter results, because these test
statistics performed worse than the usual likelihood ratio test (for
...
presentation involving non-ordinal variables, see \citeNP{StrKop13}),
