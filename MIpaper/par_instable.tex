...
although the asymptotically equivalent (see \citeNP{Sat89}) Lagrange multiplier
(LM) and Wald tests may also be constructed for this purpose. However, if $\nu$
is unknown (as is often the case for continuous $V$), then the standard theory
cannot be easily applied and instead nonstandard inference methods are required
such as the tests proposed in this paper. \readme{Slightly rephrase but probably
still not optimal.}
...
the parameters are estimated separately, potentially with some parameters
restricted to be equal across subgroups. For ease of exposition we present
...
example section. \readme{Maybe give the explicit figure reference?}
...
${\bm B}(\hat {\bm \theta})_{ij}$ that are restricted/stable and only retaining those
that are potentially instable. \readme{I guess we should say something like
this in the theory section. Is this intelligible enough?}
...
score process may still be valuable for gaining some insights.  \readme{Maybe give figure
...
\fixme{I think it would be useful to say early on that a single static
data set is drawn from the model first for illustration -- and subsequently
a full simulation study using the same scenario is considered.}
...
(the standard errors were obtained by fitting
  ...
  ``older'' subjects).
\fixme{This wasn't fully clear to me before and I probably should
have asked earlier. I think we should employ the asymptotic standard
errors divided by $\sqrt{n}$.}
...
\fixme{I think the paragraph above should say something like ``If we
knew or suspected which coefficients are subject to change\dots'' or
something else along those lines. Just to make clear that we assume
additional knowledge here. Also it may help to refer back to the
paragraph that I added in the theory section about partial changes.}
...
\readme{For PATHMOX, I would just use the argument that it is not a formal
test. Whether or not it is computationally more expensive would only be
a second argument, but I would probably avoid to use it.}
