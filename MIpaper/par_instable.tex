...
\readme{Achim: You previously modified the above section so that $\theta$
  ...
  with this, but many applications of LR tests with these models have
...
\subsection{Results}
% MODIFY RESULTS TO DISCUSS ALL FOUR TESTS
...
measurement invariance violations in applied contexts.  The specific
statstics' are not equivalent to one another, and we found that the
Cram\`{e}r-von Mises statistic exhibited the best performance for the
data generated here.  In the discussion, we consider the extent to
which these findings may extend to other factor-analytic situations.

% Need to modify table and add figure
\begin{table*}
