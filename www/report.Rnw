...
\title{\vspace*{-0.4cm} A Generalization of Classical Measurement Invariance Tests for Factor Analysis}
\Plaintitle{A Generalization of Classical Measurement Invariance Tests for Factor Analysis}
...
\Abstract{
  ...
  parameters. In this paper, we construct tests of parameter invariance based on
...
maximum $\mathit{LR}$ statistic is not $\chi^2$ anymore. However, \cite{And93}
...
% invariance.   and
% C4.5 \citep{Qui93}.  The algorithm would proceed via the following
% steps: 
% \begin{enumerate}
...
% developed by \cite{ZeiHot08} for representing
