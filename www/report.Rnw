...
\usepackage{amsmath,bm,thumbpdf}
%% need no \usepackage{Sweave}
\SweaveOpts{engine = R, eps = FALSE, echo = FALSE, results = hide}

\newcommand{\argmax}{\operatorname{argmax}\displaylimits}

\title{\vspace*{-0.4cm} Generalized Measurement Invariance Tests with
  Application to Factor Analysis}
\Plaintitle{Generalized Measurement Invariance Tests with Application
  to Factor Analysis}
\Shorttitle{Generalized Measurement Invariance Tests for Factor Analysis}

\author{Edgar C. Merkle\\Wichita State University
   \And Achim Zeileis\\Universit\"{a}t Innsbruck}
\Plainauthor{Edgar C. Merkle, Achim Zeileis}
...
\Address{
  ...
  Wichita, KS 67260-0034, United States of America\\
  E-mail: \email{Edgar.Merkle@wichita.edu}\\
  URL: \url{http://psychology.wichita.edu/merkle/}\\
...
some subset of ${\bm \theta}$; see \citealp{Mer93}) is equal
...
\citealp{Jor71,Bol89}) coupled with likelihood ratio (LR) tests are most common,
...
general psychometric analyses \citep{MacZha02}.
...
maximum $\mathit{LR}$ statistic is not $\chi^2$ anymore. \cite{And93}
...
as $\max \mathit{LR}(\nu)$ and are asymptotically equivalent \citep{And93}.
...
tests'' (see e.g., \citealp{BroDur75,And93}) where the focus of interest is the
detection of parameter instabilities of time series models ``along'' time.
Specifically, it has been shown (e.g., \citealp{Nyb89,Han92,HjoKon02,ZeiHor07})
...
change methods include \cite{HjoKon02} and \cite{ZeiHor07}.
...
of $\hat {\bm \theta}$), \cite{HjoKon02} show
that 
...
aggregation to the asymptotic process ${\bm B}^0$ \citep{HjoKon02,ZeiHor07},
...
change literature (see e.g., \citealp{Zei05,ZeiSha10}) but the three
...
so-called ``partial structural changes'' \citep{And93}. This refers to the case of some
...
\subsection[Critical values and p values]{Critical values and $p$~values}
...
of the asympotic process ${\bm B}^0(t)$ \citep{HjoKon02,ZeiHor07}. Thus, $\mathit{DM}$
...
processes (see e.g., \citealp{ShoWel86}). For all other cases, tables of critical
values can be obtained from direct simulation \citep{Zei06} or in combination with
more refined techniques such as response surface regression \citep{Han97}.
...
(see e.g., \citealp{ShoWel86,PloKra92}) with a straightforward Bonferroni
...
For the Cram\'{e}r-von Mises test statistic $\mathit{CvM}$, \cite{Nyb89}
and \cite{Han92} provide small tables of critical values which
have been extended in the software provided by \cite{Zei06}.
Critical values for the distribution of the maximum LR/Wald/LM tests
are provided by \cite{Han97}. Note that the distribution depends on
...
possible combinations of thresholds (\citealp{ZeiSha10} adapt a dynamic programming
...
In particular, \cite{Mil05} cites
...
\begin{figure}[t!]
\begin{center}
\includegraphics[width=0.78\textwidth]{famod.pdf}
...
Consider the factor analysis model in Figure~\ref{fig:famod}, which
includes six manifest variables, two 
correlated factors, and a single auxiliary variable $V$.  Though an
applied framework is unnecessary for the example, we may imagine that
six scales have been administered to students aged 13 to 18
years, with three of the scales intended to measure verbal ability and
three of the scales intended to measure mathematical ability.  We
assume a measurement invariance violation for the factor loading
parameters, whereby the factor loadings for individuals with low
values of $V$ are smaller than the factor loadings for individuals
with high values of $V$ (e.g.,  
factor loadings for older students are larger than those for younger
students).  We generally wish to assess measurement invariance with
respect to $V$ (student age).

\subsection{Method}

The base factor analysis model, displayed in
Figure~\ref{fig:famod} with set parameter values, specifies that
...
This violation states, e.g., that the verbal ability scales
...
from the overall $k = 19$~model parameters (including means) are assessed.
...
<<gefp-3-19, fig=TRUE, height=8, width=6>>=
...
@
\caption{Three test statistics of~\eqref{eq:exh0} (with $k^* = 3$)
...
\caption{A comparison of the $\max \mathit{LM}$ (solid line) and $\max \mathit{LR}$ (dashed line) test
  statistics for~\eqref{eq:h0} (i.e., $k^* = 19$).  The solid horizontal line
...
\begin{center}
\setkeys{Gin}{width=0.8\textwidth}
<<gefp-lambdas, fig=TRUE, height=8, width=6>>=
...
\caption{Cumulative score processes for each verbal factor
  ...
  correspond to the critical value at $\alpha= 0.05$.
\label{fig:lambdas}}
\end{center}
\end{figure}
...
\subsubsection[Estimation of nu and parameter isolation]{Estimation of $\nu$ and parameter isolation}
...
invariance violation for the third
...
examine the tests' power and Type I error rates in the context also
employed for the previous example.
...
tests involving only the $k^* = 3$ parameters that changed with tests of all
$k^* = k = 19$ model parameters.  The example implied that the former tests had more
...
\subsection{Method}
Data were generated analogously to the example section.
The data were generated from a factor analysis model with two
correlated factor and six manifest variables, with individuals with
low $V$ (13--15) having smaller factor loadings than individuals with
large $V$ (16--18).
...
\caption{Simulated power for three test statistics
  ...
  parameters $k^*$. Abbreviations: CvM = Cram\'{e}r-von Mises 
...
invariance (see, e.g., \citealp{Mer93}), or (2)~test a single subset of
...
(see \citealp{ZeiSha10}).  We plan to explore these issues in the future.
...
$(C-1)$ degrees of freedom \citep{HjoKon02}.
...
classification and regression trees \citep{BreFri84,MerSha10,StrMal09},
with related algorithms being developed for general parametric models
\citep{ZeiHot08} and Rasch models in particular \citep{KopZei10}.  

Relatedly,
\cite{San09} describes a general
...
in item response models (e.g., \citealp{KopZei10}, who focused on recursive
...
some treatments \citep{Mcd99} and recent research \citep{StaChe06} have
...
{lavaan}~\Sexpr{packageDescription("lavaan")["Version"]} \citep{lavaan11} and
{OpenMx}~\Sexpr{packageDescription("OpenMx")["Version"]} \citep{BokNea11} for fitting of the factor analysis models and
