...
\frame{
  \frametitle{Studying Measurement Invariance: Age $\leq$ 16}
...
\frame{
  \frametitle{Studying Measurement Invariance: Age $>$ 16}
  % path diagram illustrating parameters being the same for everyone
  \begin{center}
...
\frame{
  ...
  \begin{itemize}
    \item If we did not know the groups in advance, we could conduct a LRT wrt for each possible grouping, then take the maximum.  The proposed tests are related to this idea.
  \begin{center}
      \includegraphics[height=2.65in]{paper-lm-lr-19.pdf}
  \end{center}
  \end{itemize}}
