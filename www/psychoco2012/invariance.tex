...
% TODO: Add something like Fig 3 from paper to show test stat construction?
...
% TODO: Overlays with different parameter values on graphic
\frame{
  \frametitle{Studying Measurement Invariance}
  % path diagram illustrating parameters being the same for everyone
  \begin{center}
      \includegraphics[height=3in]{famod.pdf}
  \end{center}}
...
\frame{
  ...
  \begin{itemize}
    ...
    \item Hypothesis of ``full'' measurement invariance ($\bm{\theta}_i$ is the full parameter vector for individual $i$):\\
      \begin{center}
...
% TODO Consistent notation with paper
\frame{
  \frametitle{Proposed Tests}
  \begin{itemize}
  ...
  \end{itemize}
\begin{eqnarray*}
  \displaystyle\sum_{i=1}^{n} \psi(\bm{x}_i, \hat{\bm{\theta}}) =
  {\bm 0},\text{ where} \\
 \\
  \psi(\bm{x}_i, \hat{{\bm \theta}}) = \frac{\partial}{\partial \bm{\theta}} \log
...
\frame{
 ...
 % define cumulative scores and other notation
  ...
  \end{itemize}
  \begin{center}
    $\bm{W}_n(t,\bm{\theta}) = \frac{1}{\sqrt{n}}
 ...
 % show that cumulative scores follow a Brownian bridge under H_0
  \begin{itemize}
    \item Theorem (Zeileis \& Hornik, 2007): Under the hypothesis of
      ...
      theorem holds:
  \begin{equation}
  ${\bm{I}}(\widehat{{\bm{\theta}}})^{-1/2}{\bm{W}}_n(\cdot,\widehat{{\bm{\theta}}}) 
\overset{d}{\rightarrow} {\bm W}^{0}(\cdot),$
  \end{equation}
 where ${\bm{I}}(\widehat{{\bm{\theta}}})$ is the observed
 information matrix and ${\bm W}^{0}(\cdot)$ is a
 $p$-dimensional Brownian bridge (where $p$ is the number of parameters).
  \end{itemize}
}
...
% TODO: Break up results over multiple slides
\frame{
  \frametitle{Results}
  \begin{center}
      \includegraphics[height=3in]{paper-gefp-4-1.pdf}
  \end{center}}
...
% TODO: Add some detail on software
\frame{
  \frametitle{Software}
  \begin{itemize}
    \item To carry out the tests, we use
      \begin{itemize}
        \item \texttt{lavaan} for model estimation.
        \item A combination of \texttt{lavaan} and our own code for calculating scores of fitted models.
...
\frame{
  ...
  \begin{itemize}
    ...
    \item Extension to related psychometric issues (modification
      indices, mediation).
   % TODO: URL for working paper
  \end{itemize}}
