...
maximum $\mathit{LR}$ statistic is not $\chi^2$ anymore. Andrews \citeyear{And93}
...
Using results from Hjort and Koning \citeyear{HjoKon02} and Zeileis
and Hornik \citeyear{ZeiHor07}, we can also
...
of deviation $\bm{theta}(v_i)$ can be described as a constant
...
Under the local alternative described above, Hjort and Koning
\citeyear{HjoKon02} show that the process
...
data from Wicherts, Dolan, and Hessen \citeyear{WicDol05}.
Following the example, we consider a
full simulation study involving artificial data.

\section{Example: Stereotype Threat}

Wicherts, Dolan, and Hessen \citeyear{WicDol05} utilized confirmatory
...
ethnic minorities' performance on tests of general intelligence.  In this section, we used the proposed tests to supplement the authors' original analyses.

\subsection{Background}

To study stereotype threat in a measurement invariance framework,
Study 1 of Wicherts et al.\ \citeyear{WicDol05} involved 295 high school
students completing three intelligence tests in two between-subjects
conditions.  Conditions were defined by
whether or not students received primes about ethnic stereotypes prior to testing.
To study the data, Wicherts et al.\ employed a four-group,
one-factor model with the three intelligence tests as manifest variables.
The groups were defined by ethnicity (majority/minority) and by the
experimental manipulation (received/did not receive stereotype prime).
Results indicated that the intelligence tests lacked measurement
invariance, with the minority group receiving stereotype primes being
particularly different from the other three groups on the most
difficult intelligence test.  Most notably, the factor loading and
intercept for the "minority, stereotype prime" group were smaller, and the unique variance
was larger. 

As Wicherts et al. describe, one explanation for this pattern of results involves the idea that ability is positively related to "domain identification" \cite{Ste97}: individuals who are high on ability may be more prone to stereotype threat because they are more "tied in" to the domain of interest and are more aware of stereotypes.  To test this idea, we employ a four-group, confirmatory factor model used by Wicherts et al.\ in tandem with the tests proposed in this paper.  Our $V$ variable is participants' grade point averages (which were unused in the original analyses), which will reflect participants' identification with the academic domain.
...
modification indices.  Most notably, the most difficult intelligence test's factor loading, unique variance, and intercept differed for the "minority, stereotype threat" group, as compared to the other groups.
Factor means were also estimated for three of the groups.  The model
provided a reasonable fit to the data, as judged by examination of
$\chi^2$, RMSEA, and CFI statistics.  Wicherts et al.\ also had a Step
6 (the final step), where they examined a model that was simpler than
described above and that provided a slightly better fit to the data.
This model restricted factor means to be equal across a subset of the
groups, however, which was not helpful for our purposes (because we are focusing on a single group).

The stereotype threat effect of interest is most relevant to the
"minority, stereotype prime" group.  Thus, to test the idea that
domain identification impacts the magnitude of stereotype threat
effect, we focus on parameters specific to this group.  We test these
parameters for measurement invariance across the GPA variable, with
violations supporting the hypothesis that domain identification
impacts the magnitude of the stereotype threat effect.  We also test
the intercept parameter individually.  This is because, if domain
identification does impact stereotype threat, it could be
conceptualized as the error term having a nonzero expectation across
individuals (i.e., individuals with high domain identification have a
negative error expectation and individuals with low domain
identification have a positive error expectation).  The expected value
for the error term cannot be uniquely separated from the intercept, so
that the estimated intercept would display a lack of measurement
invariance (see, e.g., \citeNP{Mil11}, p.\ 77).  

To carry out
the tests, the four-group confirmatory factor model described above
was first fit to the data.  Casewise derivatives and 
...
\begin{equation}
    \label{eq:exh0}
H_0:\ (\nu_{i,1,\text{MSP}}\ \lambda_{i,1,\text{MSP}}\
\theta_{i,1,\text{MSP}}\ \alpha_{i,1,\text{MSP}}) = 
(\nu_{0,1,\text{MSP}}\ \lambda_{0,1,\text{MSP}}\
\theta_{0,1,\text{MSP}}\ \alpha_{0,1,\text{MSP}}),\ i=1,\ldots,n_{\text{MSP}},
\end{equation}
where $\nu_{i,1,\text{MSP}}$ reflects the intercept for student $i$ on intelligence
test 1 in the ``minority, stereotype prime'' group, with other
subscripts defined similarly.  Additionally, $\lambda$ is a factor loading, $\theta$
is a unique variance, and $\alpha$ is a factor mean, and the
hypothesis is taken only wrt the $n_{\text{MSP}}$ individuals in the
``minority, stereotype prime'' group.  In words, the hypothesis states
that the four group-specific model parameters are invariant with
respect to GPA.
...
\begin{equation}
    \label{eq:exh0.2}
H_0:\ \nu_{i,1,\text{MSP}}\ = \nu_{0,1,\text{MSP}},\ i=1,\ldots,n_{\text{MSP}},
\end{equation}
which, as described previously, tests a more specific theory of the
manner in which domain identification impacts stereotype threat.
...
\subsection{Results}
<<<mz_mod>>=
d <- dget("WichertsEtAl2005_Study1.txt")

wdh0 <- wdhfit(d)

## subsample models
## This could be used to compare sequential LRTs with the LM
## statistic that tests all parameters simultaneously.
## (This is not done in the data example.)
##############################################################
## lrstat <- function(gpa) 2 * as.vector(logLik(wdhfit(d[d$gpa <= gpa,])) + logLik(wdhfit(d[d$gpa > gpa,])) - logLik(wdh0))
## wdh.gpa <- sort(d$gpa[d$gpa >= quantile(d$gpa, 0.1) & d$gpa <= quantile(d$gpa, 0.9)])
## # Results in many "computationally singular" errors:
## wdh.lrstat <- sapply(wdh.gpa, lrstat)
## save(wdh0, wdh.gpa, wdh.lrstat, file = "wicherts_mod.rda")
@ 

Test statistics for the hypotheses \eqref{eq:exh0}
and~\eqref{eq:exh0.2} are displayed in Figure~\ref{cusumex}.
...
The results displayed in Figure~\ref{cusumex} generally show that the
``minority, stereotype prime'' group lacks invariance with respect to
GPA.  All three tests are significant at the .05 level when the four
group-specific parameters are tested simultaneously (first column).
When the intercept parameter is tested individually, two of the three
tests achieve significance (the CvM test and $\max \mathit{LM}$
statistics).  These results generally support the notion that domain
identification (as measured by GPA) influences the magnitude of
stereotype threat.  The $k*=1$ results also imply that the tests
differ in power and/or in error rates, an issue we address later in the
paper.

%% add paragraph about peaks of graphs
...
panels (first column), two peaks occur near GPAs of 6.0 and of 6.3 (6
...
the second column of Figure~\ref{fig:cusumex}.  Here, a single peak
occurs in the three panels around a GPA of 7.0 (`B' range).  
...
## add small amount of random noise to duplicated GPAs, so that everything
## works with zoo.  (Without this, one can still obtain DM and CvM test 
##                   results but not supLM results.)
set.seed(1090)
ties <- duplicated(d$gpa)
d$gpa[ties] <- runif(sum(ties), d$gpa[ties]-.01, d$gpa[ties]+.01)
## empirical fluctuation processes (4 vs 1 par)
gefp_4_info <- gefp(wdh0, fit = NULL, order.by = d$gpa[d$grouping=="exp:minority"], parm = 15:18, vcov = info.wdhfit, sandwich = FALSE)
gefp_1_info <- gefp(wdh0, fit = NULL, order.by = d$gpa[d$grouping=="exp:minority"], parm = 18, vcov = info.wdhfit, sandwich = FALSE)
...
Wicherts et al.\ \citeyear{WicDol05} data allowed us to study the
domain identification's impact on stereotype threat in a
straightforward manner.  The tests focused on a set of group-specific
...
described here may adversely impact model fit and degrees of freedom (e.g., if
GPA is inserted directly into the model) or require multiple models to
be estimated (e.g., for LRTs).  Further, through examination of both
test statistics and cumulative score processes, the tests were easily
...
\caption{Simulated power curves for the double-max test (solid), 
  ...
  across three sample sizes $n$, two subsets of tested parameters $k^*$, and
...
mykey$lines$pch <- c(2, 3, 1)
mykey$lines$lty <- c(1.2, 2, 1.2)
...
\caption{Simulated power for three test statistics
  across three sample sizes $n$, nine magnitudes of measurement
...
mz_tab <- c(mz_tab,"\\multicolumn{12}{l}{{\\scriptsize{Abbreviations: CvM = Cram\\'{e}r-von Mises 
  test; $\\max \\mathit{LM}$ = Maximum Lagrange multiplier test; DM = Double-max test.}}}")
writeLines(mz_tab)
@
...
with increased complexity (due to the moderator/covariate).
...
interpreting models of greater complexity.  This may not be a
...
observations.  For 
example, it would be possible to assess whether paths between
...
in item response models (e.g., \citeNP{KopZei10}, who focused on recursive
