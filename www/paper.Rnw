...
0.5, \dots, 4$.  The 0-standard error condition was
...
\label{fig:simres}
\setkeys{Gin}{width=\textwidth}
<<mzsim-xyplot, fig=TRUE, height=7, width=7>>=
## visualization
trellis.par.set(theme = canonical.theme(color = FALSE))
print(xyplot(power ~ diff | pars + nobs, group = ~ test, data = mz_sim_sub, type = "b",
...
levels(mz_sim_sub$test) <- gsub("supLM", "max-LM", levels(mz_sim_sub$test), fixed = TRUE)
