...
\acknowledgements{This work was supported by National Science
  ...
  tests with the lavaan package, Kris Preacher, who provided helpful
  comments 
  on the manuscript, and the participants of the
...
\begin{equation} \label{eq:lr}
  \mathit{LR}(\nu) ~=~ -2 \left[
   ...
   ~-~ \{\ell(\hat {\bm \theta}^{(A)}; x_1, \dots, x_m)
    ...
    \right],
\end{equation}
which, when the null hypothesis holds, has an asymptotic $\chi^2$ with degrees
...
theoretical result involves the fact that, under $H_0$ in \eqref{eq:h0}, the
...
decorrelated scores.  The second row of ${\bm B}(\hat {\bm \theta})$ corresponds to
...
if any component of the cumulative score process ${\bm B}(t; \hat {\bm \theta})$
strays ``too far'' from zero at any time, i.e., if the double maximum statistic
\begin{equation}
    \label{eq:dmax}
...
(in the ``Example'' section) which shows the cumulative score processes for three
...
cases, sums instead of maxima are more suitable for collapsing across
...
restricted/stable, retaining only those $k^*$ columns that are potentially unstable.
...
of the asymptotic process ${\bm B}^0(t)$ \cite{HjoKon02,ZeiHor07}. Thus, $\mathit{DM}$
from (\ref{eq:dmax}) converges to $\sup_t || {\bm B}^0(t) ||_{\infty}$, where
$||\cdot||_\infty$ denotes the maximum norm. Similarly, $\mathit{CvM}$
from (\ref{eq:cvm}) converges to $\int_0^1 || {\bm B}^0(t) ||_2^2 d t$, where $||\cdot||_2$
...
capture the behavior of the process $\bm{B}(t, \hat{\bm{\theta}})$
...
statistic $\mathit{CvM}$ (test statistics for the others are simply 
...
measurement invariance violation as was described in the ``Artificial Example'' section.
...
values by $d$~times the parameters' asymptotic standard errors (scaled by $\sqrt{n}$),
with $d = 0, 0.25, 0.5, \dots, 4$.  The $d=0$ standard error condition was
...
the underlying numeric values for a subset of the results are additionally displayed in
...
Our $V$ is participants' aptitudes, as measured by their grade point averages (GPAs) which were unused in the original analyses.
...
## fit model 5b
wdh5b <- lavaan(
   'ability   =~ c(rep("load_n", 3), "load_n:min_t") * numerical + rep("load_v", 4) * verbal + 1 * abstract
    ability   ~  c(NA, "lmean:min_c", "lmean:maj_t", "lmean:min_t") * 1 + c(0, NA, NA, NA) * 1
    ability   ~~ c("lvar:maj", "lvar:min", "lvar:maj", "lvar:min") * ability
    numerical ~  c(rep("mean_n", 3), "mean_n:min_t") * 1
    abstract  ~  c("mean_a:maj_c", "mean_a:min_c", rep("mean_a", 2)) * 1
    verbal    ~  rep("mean_v", 4) * 1
    numerical ~~ c(rep("var_n", 3), "var_n:min_t") * numerical
    abstract  ~~ rep("var_a", 4) * abstract
    verbal    ~~ rep("var_v", 4) * verbal',
  data = wdh, meanstructure = TRUE, group = "group") ##, likelihood = "wishart")

## refit model with new identification constraints
wdh5b_1 <- lavaan(
   'ability   =~ 1 * verbal + c(rep("load_n", 3), "load_n:min_t") * numerical  + rep("load_a", 4) * abstract
    ability   ~  c(NA, "lmean:min_c", "lmean:maj_t", "lmean:min_t") * 1 + c(0, NA, NA, NA) * 1
    ability   ~~ c("lvar:maj", "lvar:min", "lvar:maj", "lvar:min") * ability
    numerical ~  c(rep("mean_n", 3), "mean_n:min_t") * 1
    abstract  ~  c("mean_a:maj_c", "mean_a:min_c", rep("mean_a", 2)) * 1
...
\caption{Cumulative score processes for the four group-specific parameters across GPA
  ...
  using data from Study 1 of WDH. %% \citeA{WicDol05} does not work in caption
  The solid gray horizontal lines correspond to the critical value of
  the double-max test at $\alpha = 0.05$.}
\label{fig:cusumgpa}
\setkeys{Gin}{width=0.8\textwidth}
...
class of moderated factor models \cite{BauHus09,MolDol10,Nea06,Pur02}, 
whereby continuous moderators are allowed to linearly impact model
parameters (\citeNP{Pur02}, also considers quadratic effects of the
