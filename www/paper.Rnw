...
other three groups on the most difficult intelligence test.  Most notably, the
factor loading and intercept for the ``minority, stereotype prime'' group were
smaller, and the unique variance was larger. 

As Wicherts et al. describe, one explanation for this pattern of results involves the idea that ability is positively related to ``domain identification''
\cite{Ste97}: individuals who are high on ability may be more prone to stereotype threat because they are more ``tied in'' to the domain of interest and are
more aware of stereotypes.  To test this idea, we employ a four-group, confirmatory factor model used by Wicherts et al.\ in tandem with the tests proposed
in this paper.  Our $V$ variable is participants' grade point averages (GPA, which were unused in the original analyses), which will reflect participants' identification with the academic domain.
...
modification indices.  Most notably, the most difficult intelligence test's factor loading, unique variance, and intercept differed for the ``minority,
stereotype threat'' group, as compared to the other groups.
...
(comparative fit index) statistics.  Wicherts et al.\ also had a
Step~6 (the final step), where they examined a model that was simpler than
...
\citeA{WicDol05} data allowed us to study the
