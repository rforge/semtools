...
\abstract{
  ...
  The tests are presented and illustrated in detail, along with an application
...
\acknowledgements{This work was supported by National Science
  ...
  generously shared data for the stereotype threat example.
...
observations $\bm{x}_i$. This situation is also generally encountered in structural
equation models with missing data (e.g., \citeNP{Wot00}).

\readme{maybe like this?}
...
(LM) tests (also known as score tests) and Wald tests may also be constructed for this purpose (see
\citeNP{Sat89}). If $\nu$
...
\fixme{Explain what the 19 parameters are.}
...
\caption{Cumulative score processes for each verbal factor loading. 
  The horizontal lines correspond to the critical value of the
...
the same processes aggregated over the three parameters. In both graphics, horizontal lines reflecting the
...
is very close to its nominal type~I error rate, with the double-max test being somewhat
...
\readme{now called ``Application'' as opposed to the artificial ``Example''}
...
\fixme{Include path diagram and a bit more background so that the paper is more
accessible for readers that are not familiar with the WDH paper.}
...
Our $V$ variable is participants' grade point averages (GPAs), which were unused in the original analyses.
\fixme{Maybe add: ``\dots as a measure of aptitude.'' Or something along these lines? I'm not
sure what the appropriate psychometric terminology is.}
...
calculated the cumulative score process with respect to student GPA via
...
As mentioned in the theory section, the 
tests give us the flexibility to study hypotheses of partial change. That is, we
...
\end{equation}
where
$\lambda_{i, \text{num}}$ is the factor loading of student $i$'s intelligence on the numerical scale,
...
The students are ordered according to GPA (i.e.,  in this application the variable $V$),
and the  hypothesis is taken only with respect to the $n_{\text{MSP}}$ individuals in the
``minority, stereotype prime'' group.  Thus, \eqref{eq:exh0} states  that these four group-specific
model parameters are invariant with respect to GPA.

\readme{I have omitted the single parameter hypothesis. Still it would be good to
double-check the estfun method once more, though.}
...
with this parameter. However, also the factor loading for the numerical scale
$\lambda_\text{num}$ and the factor mean $\eta$ exhibit increased fluctuation
albeit they are just non-significant at level $\alpha = 0.05$. Only the process
associated with the intercept $\mu_\text{num}$ just shows moderate (and hence
...
\caption{Cumulative score processes for the four group-specific parameters across GPA
  ...
  The horizontal lines correspond to the critical value of
...
loading is fixed to 1 \fixme{figure ref} and compared them to the p-values
...
Furthermore, in this situation, one could also adapt the statistics
\eqref{eq:dmax}, \eqref{eq:cvm}, and \eqref{eq:maxlm} computed from the same cumulative score
process as usual. The only modification required should be that the statistics
...
all observations $i = 1, \dots, n$ but only those $i$ at the ``end'' of a
category.
...
\readme{Is this intelligible or do we need more information?}
