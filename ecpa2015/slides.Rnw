...
scores cannot be reliably attributed to the latent variable that the model
purports to measure.
...
\begin{itemize}
  ...
  \item Numerical variables are often categorized into groups in an ad hoc way
...
\textbf{Idea:}
\begin{itemize}
  \item Generalize the LM test (or score test).
...
\begin{itemize}
  \item Model with two latent scales: Verbal and math score.
